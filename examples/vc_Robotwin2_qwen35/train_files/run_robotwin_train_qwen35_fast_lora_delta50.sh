...
#
# Usage (single-node):
#   MACHINE=l40s bash run_robotwin_train_qwen35_fast_lora_delta50.sh
#   MODE=debug   bash run_robotwin_train_qwen35_fast_lora_delta50.sh
# ---------------------------------------------------------------------------
...
if [[ "$machine" == "l40s" ]]; then
    ...
    echo "Unknown MACHINE: $machine  (choices: l40s, a100, h100, h200)"
    exit 1
fi
