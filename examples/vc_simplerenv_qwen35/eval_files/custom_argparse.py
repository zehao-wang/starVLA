...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional SAPIEN renderer device, e.g. cuda:0.",
    )
    ...
    if args.renderer_device is not None:
        ...
        if not isinstance(renderer_kwargs, dict):
            raise ValueError("renderer_kwargs must be a dict when --renderer-device is used")
